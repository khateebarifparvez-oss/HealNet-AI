...
function navigateToSection(sectionId) {
    ...
    if (targetSection) {
        ...
        // SCROLL TO TOP OF THE FORM - This fixes the middle-page issue!
        // Scroll to the diagnosis form section smoothly
        ...
        // Uncomment this if you want to scroll to absolute top instead:
...
function displayResults(analysisResult) {
    ...
    // Display risk level with NEW PROMINENT DESIGN
    const riskLevelElement = document.getElementById('riskLevel');
    
    let riskIcon = '';
    let riskTitle = '';
    let riskDescription = '';
    let riskClass = analysisResult.riskLevel.level;
    
    switch (analysisResult.riskLevel.level) {
        case 'low':
            riskIcon = '✓';
            riskTitle = 'Low Health Risk';
            riskDescription = 'Your current health profile indicates a low overall risk level. Continue maintaining your healthy habits and regular check-ups.';
            break;
        case 'medium':
            riskIcon = '⚠️';
            riskTitle = 'Medium Health Risk';
            riskDescription = 'Your health profile shows some areas that need attention. Consider implementing the recommendations below and consult with your healthcare provider.';
            break;
        case 'high':
            riskIcon = '⚠️';
            riskTitle = 'High Health Risk';
            riskDescription = 'Your health profile indicates several risk factors that require attention. We recommend consulting with healthcare professionals promptly.';
            break;
    }
    
    // Create the PROMINENT risk level box HTML
    const riskMessage = `
        <div class="risk-level-box ${riskClass}">
            <span class="risk-level-icon">${riskIcon}</span>
            <h3>Your Health Risk Assessment</h3>
            <div class="risk-level-badge ${riskClass}">${riskTitle}</div>
            <p class="risk-level-description">${riskDescription}</p>
        </div>
    `;
